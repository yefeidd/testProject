...
//                }
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                if (!mViewPortHandler.isInBoundsRight(buffer.buffer[j]))
                    break;
                int i = j / 4;
                if (dataSet.getEntryForIndex(i).getmOpen() <= dataSet.getEntryForIndex(i).getmColse()) {
                    mRenderPaint.setColor(((BarDataSet) dataSet).getIncreasingColor());

                } else {
                    mRenderPaint.setColor(((BarDataSet) dataSet).getDecreasingColor());
