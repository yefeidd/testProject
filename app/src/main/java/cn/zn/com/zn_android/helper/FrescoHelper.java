...
//                .setBaseDirectoryPath(new File(StorageUtil.getCachePath(context)))
//                .setBaseDirectoryName(Constants.FILE_CACHE_IMG_SMALL)
...
//                .build();
        ...
        //默认图片的磁盘配置
        DiskCacheConfig diskCacheConfig =  DiskCacheConfig.newBuilder(RnApplication.getInstance())
                .setBaseDirectoryPath(new File(StorageUtil.getCachePath(context)))
                .setBaseDirectoryName(Constants.FILE_CACHE_IMG)
//            .setCacheErrorLogger(cacheErrorLogger)//日志记录器用于日志错误的缓存。
//            .setCacheEventListener(cacheEventListener)//缓存事件侦听器。
//            .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
                .setMaxCacheSize(FrescoHelper.MAX_DISK_CACHE_SIZE)//默认缓存的最大大小。
...
//    }
  ...
  private static ImagePipelineConfig configureCaches(Context context) {
    ...
    //小图片的磁盘配置
      DiskCacheConfig diskSmallCacheConfig =  DiskCacheConfig.newBuilder(RnApplication.getInstance())
...
//              .setFailureImage(FrescoHelper.sErrorDrawable)//fresco:failureImage="@drawable/error"失败图
...
//              .setPlaceholderImage(FrescoHelper.sPlaceholderDrawable)//fresco:placeholderImage="@color/wait_color"占位图
